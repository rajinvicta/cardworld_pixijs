...
class CalculateDeps {
  ...
  _emit(dest) {
    ...
    let destText = fs.readFileSync(dest, "utf8");
...
class CalculateDepsPlugin {
  constructor(src, dest, single, ignore) {
    this.config = {
      src: src,
      dest: dest,
      single: single,
  ...
  apply(compiler) {
    ...
    compiler.hooks.watchRun.tap('CalculateDepsPlugin', () => {
      console.log("Watch Executed!");
      let calcDeps = new CalculateDeps();
      let src = this.config.src;
      let dest = this.config.dest;
      let single = this.config.single;
      let ignore = this.config.ignore;


      calcDeps.refreshDeps(src, dest, single, ignore);
...
module.exports = CalculateDepsPlugin;